...
import { Descriptions, Tag, Timeline, Button, Form, Input, DatePicker, Select, message, Space, Card, Modal, Tooltip } from 'antd';
import { ArrowLeftOutlined, PlusOutlined, SaveOutlined, SwapOutlined, UserOutlined } from '@ant-design/icons';
...
const OpportunityDetail: React.FC = () => {
  ...
  if (!opp) return <div>{loading ? '加载中...' : '商机不存在'}</div>;
  ...
  return (
    <div>
      ...
      <Card title="基本信息" extra={
        <Space>
          {!editMode && user.role === 'admin' && opp.sales_name && (
            <Button icon={<SwapOutlined />} size="small" onClick={() => setReassignModalVisible(true)}>转交销售</Button>
          )}
          {canEdit && !editMode ? <Button onClick={() => setEditMode(true)}>编辑</Button> : null}
        </Space>
        ...
        {editMode && user.role === 'admin' ? (
          <Form form={form} layout="vertical" style={{ maxWidth: 600 }}>
            ...
            {user.role === 'admin' && (
              <Form.Item name="sales_id" label="分配销售">
                <Select placeholder="选择销售负责人" allowClear options={salesList.map(s => ({ value: s.id, label: s.name }))} />
              </Form.Item>
            )}
            <Space>
          ...
          <Descriptions column={2} bordered size="small">
            ...
            <Descriptions.Item label="当前状态"><Tag color={statusColors[opp.status]}>{opp.status}</Tag></Descriptions.Item>
            ...
            <Descriptions.Item label="创建时间">{dayjs(opp.created_at).format('YYYY-MM-DD HH:mm')}</Descriptions.Item>
            <Descriptions.Item label="更新时间">{dayjs(opp.updated_at).format('YYYY-MM-DD HH:mm')}</Descriptions.Item>
          </Descriptions>
        )}
      </Card>
      <Card title="跟进记录" extra={canEdit ? <Button type="primary" icon={<PlusOutlined />} onClick={() => setFollowupModalVisible(true)}>添加跟进</Button> : null}>
        {followups.length === 0 ? <div style={{ color: '#999', textAlign: 'center', padding: 20 }}>暂无跟进记录</div> : (
          <Timeline items={followups.map((f: Followup) => ({
            children: (
              <div>
                <div style={{ fontWeight: 500 }}>{dayjs(f.followup_time).format('YYYY-MM-DD HH:mm')}
                  {f.status && <Tag color={statusColors[f.status]} style={{ marginLeft: 8 }}>{f.status}</Tag>}
                </div>
                <div style={{ marginTop: 4, color: '#333' }}>{f.content}</div>
      ...
      <Modal title="添加跟进记录" open={followupModalVisible} onCancel={() => setFollowupModalVisible(false)}
        ...
        <Form form={followupForm} layout="vertical">
          ...
          <Form.Item name="content" label="跟进内容" rules={[{ required: true, message: '请输入跟进内容' }]}>
            <Input.TextArea rows={4} placeholder="请输入本次跟进的详细内容..." />
          </Form.Item>
      ...
      <Modal title={`转交销售 - ${opp?.company_name || ''}`} open={reassignModalVisible}
        ...
        okText="确认转交" cancelText="取消" width={450}>
